...
def refine_mesh(data: MdpaData, subdivisions: int) -> MdpaData:
    ...
    # Remember which original SMPs had conditions
    original_smps_with_conditions = set()
    for smp_name, smp in data.sub_model_parts.items():
        if smp.conditions:
            original_smps_with_conditions.add(smp_name)

    # =====================================================
    # 1. Create refined nodes
    # =====================================================
    ...
    # =====================================================
    # 3. One PointLoadCondition3D1N per node
    # =====================================================
    all_node_ids = sorted(refined.nodes.keys())
    for i, node_id in enumerate(all_node_ids, start=1):
        refined.conditions[i] = {'property': 0, 'nodes': [node_id]}

    all_cond_ids = list(range(1, len(all_node_ids) + 1))

    # =====================================================
    # 4. Build SubModelParts
    # =====================================================
    for smp_name, smp in data.sub_model_parts.items():
        ...
        # Map elements and add intermediate nodes
        for old_elem_id in smp.elements:
            if old_elem_id in old_to_new_elements:
                ...
                # Add intermediate nodes from these elements
                for new_elem_id in old_to_new_elements[old_elem_id]:
                    new_smp.nodes.extend(refined.elements[new_elem_id]['nodes'])

        # If this SMP originally had conditions, give it ALL conditions
        if smp_name in original_smps_with_conditions:
            new_smp.conditions = list(all_cond_ids)
            new_smp.nodes.extend(all_node_ids)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        sys.exit(1)
    refine_mdpa(sys.argv[1], sys.argv[2], int(sys.argv[3]) if len(sys.argv) > 3 else 2)