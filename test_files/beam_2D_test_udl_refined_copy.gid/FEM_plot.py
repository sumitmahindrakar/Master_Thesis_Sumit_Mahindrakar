...
VTK_FILE_PATH = "test_files/beam_2D_test_udl_refined_copy.gid/vtk_output/Parts_Beam_Beams_0_1.vtk"  # <-- Change this to your VTK file path
...
OUTPUT_FOLDER = "test_files/beam_2D_test_udl_refined_copy.gid/plots"     # Folder to save output figures
...
def add_roller_supports(ax, points, support_indices=None):
    """
    Add support symbols at fixed nodes.
    ...
    for idx in support_indices:
        ...
        roller = plt.Circle((x, y - size_2*1.5), 
                        size_2/2, facecolor='white', edgecolor='black', lw=2)
        ax.add_patch(roller)
...
def create_frame_plots(vtk_data, save_figures=SAVE_FIGURES, 
    ...
    add_supports(ax1, points, support_indices=[0])
    add_roller_supports(ax1, points, support_indices=[2])
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ...
    add_supports(ax2, points, support_indices=[0])
    add_roller_supports(ax2, points, support_indices=[2])
    ax2.set_xlabel('X (m)')
    ax2.set_ylabel('Y (m)')
    ...
    add_supports(ax3, points, support_indices=[0])
    add_roller_supports(ax3, points, support_indices=[2])
    ax3.set_xlabel('X (m)')
    ax3.set_ylabel('Y (m)')
    ...
    plot_deflection_diagram(ax, points, cells, displacement, 
                           scale=DEFLECTION_SCALE, show_fill=True)
    add_supports(ax, points, support_indices=[0])
    # print(points)
    add_roller_supports(ax, points, support_indices=[2])
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ...
    ax.text(0.02, 0.1, f'Max displacement: {max_disp:.6f} m\nScale factor: {DEFLECTION_SCALE}x',
