...
- I33 sensitivity (dJ/dI33)
- Young's modulus sensitivity (dJ/dE)
- Point load sensitivity (dJ/dq)
...
os.chdir(r"E:\Master_Thesis_Sumit_Mahindrakar\test_files\SA_Kratos_adj_V2.5")
...
def print_adjoint_results(model_part):
    ...
    try:
        ...
        for condition in load_mp.Conditions:
            try:
                P_sens = condition.GetValue(StructuralMechanicsApplication.LINE_LOAD_SENSITIVITY)
...
def print_summary(model_part):
    ...
    try:
        ...
        for element in sensitivity_mp.Elements:
            try:
                i33_sens = element.GetValue(StructuralMechanicsApplication.I33_SENSITIVITY)
                total_i22_sens += i33_sens
            except:
                pass
            try:
                E_sens = element.GetValue(StructuralMechanicsApplication.YOUNG_MODULUS_SENSITIVITY)
...
def print_vtk_info():
    ...
    print("  - Element data: I33_SENSITIVITY, YOUNG_MODULUS_SENSITIVITY")
    print("  - Condition data: LINE_LOAD_SENSITIVITY")
...
def main():
    ...
    try:
        ...
        print(f"Adjoint analysis error: {e}", flush=True)
        traceback.print_exc()
        return 1
    except:  # catches SystemExit, KeyboardInterrupt, etc.
        print("Adjoint analysis crashed unexpectedly!", flush=True)
        return 1
