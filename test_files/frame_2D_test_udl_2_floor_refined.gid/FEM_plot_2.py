...
VTK_FILE_PATH = "test_files/frame_2D_test_udl_2_floor_refined.gid/vtk_output/Parts_Beam_Beams_0_1.vtk"  # <-- Change this to your VTK file path
...
FIGURE_DPI = 100
SAVE_FIGURES = True
OUTPUT_FOLDER = "test_files/frame_2D_test_udl_2_floor_refined.gid/plots"
# --- Support Settings ---
...
def plot_deflection_diagram(ax, points, cells, displacement, 
    ...
    for idx, cell in enumerate(cells):
        ...
        ax.text(0.2, 0.1, f'Max displacement: {max_disp:.6f} m\nScale factor: {DEFLECTION_SCALE}x',
...
def create_combined_plot(vtk_data, save_path=None):
    ...
    add_supports(ax, points, displacement=displacement)
...
def create_individual_plots(vtk_data, output_folder=OUTPUT_FOLDER):
    ...
    ax.text(0.2, -0.1, f'Max displacement: {max_disp:.6e} m\nScale: {DEFORMED_SCALE}x',
    ...
    ax.text(0.35, 0.2, f'Max BM: {max_moment:.1f}\nMin BM: {min_moment:.1f}',
           transform=ax.transAxes, fontsize=10, va='top',
           bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
