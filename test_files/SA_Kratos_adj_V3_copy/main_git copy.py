...
class TestAdjointSensitivityAnalysisBeamStructureLocalStress(AdjointSensitivityAnalysisTestFactory):
    ...
    def perform_additional_checks(self):
        ...
        # self.assertAlmostEqual(sensitivities_to_check[7], reference_values[4], 5)def perform_additional_checks(self):
        ...
        # print("\n=== LINE_LOAD_SENSITIVITY ===")
        # for cond_id in range(1, 11):
        #     val = adjoint_model_part.Conditions[cond_id].GetValue(
        #         StructuralMechanicsApplication.LINE_LOAD_SENSITIVITY
        #     )
        #     print(f"  Condition {cond_id}: [{val[0]:.6f}, {val[1]:.6f}, {val[2]:.6f}]")
...
class TestAdjointSensitivityAnalysisSpringDamperElement(AdjointSensitivityAnalysisTestFactory):
    primal_file_name = "ProjectParameters.json"
    adjoint_file_name = "AdjointParameters.json"


@KratosUnittest.skipUnless(has_hdf5_application,"Missing required application: HDF5Application")
class TestAdjointSensitivityAnalysisLinearTrussStructure(AdjointSensitivityAnalysisTestFactory):
    primal_file_name = "linear_truss_test_parameters.json"
    adjoint_file_name = "linear_truss_test_local_stress_adjoint_parameters.json"


@KratosUnittest.skipUnless(has_hdf5_application,"Missing required application: HDF5Application")
class TestAdjointSensitivityAnalysisNonLinearTrussStructure(AdjointSensitivityAnalysisTestFactory):
    primal_file_name = "nonlinear_truss_test_parameters.json"
    adjoint_file_name = "nonlinear_truss_test_local_stress_adjoint_parameters.json"
