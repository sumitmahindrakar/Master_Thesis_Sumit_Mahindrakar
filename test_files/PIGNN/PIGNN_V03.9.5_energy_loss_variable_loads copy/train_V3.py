...
class TrainConfig:
    ...
    resume_from     = None  # or "RESULTS/final.pt" "RESULTS/best.pt" 
    ...
    lr              = 1e-3 # lower for fine-tuning
    warmup_epochs   = 200 # 0 -> No warm up needed for training the trained model
    decay_epoch     = 3000
    lr_decay_factor = 0.1
    weight_decay    = 0.0
...
class Trainer:
    ...
    def __init__(self, config: TrainConfig):
        ...
        with torch.no_grad():
            ...
            loss, loss_dict, _, _ = self.loss_fn(self.model, d)
            print(f"    loss: {loss.item():.6e} (should be ~1.0)")
            print(f"    U: {loss_dict['U_internal']:.6e}")
            print(f"    W: {loss_dict['W_external']:.6e}")
    ...
    def _load_data(self):
        ...
        # Target energies → target is now 0 for equilibrium
        self._avg_target = 0.0  # equilibrium residual target
        print(f"  Target: Loss → 0 (equilibrium residual)")
    ...
    def _load_checkpoint(self):
        if hasattr(self.cfg, 'resume_from') and self.cfg.resume_from:
            ...
            if os.path.exists(path):
                ...
                self.best_Pi = ckpt.get('best_Pi', float('inf'))
                print(f"  Resumed from {path} (epoch {ckpt['epoch']})")
                print(f"  Best Π: {self.best_Pi:.6e}")
                return True
        self.start_epoch = 1
        return False
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_data):
                data = data.to(self.device)
                pred_phys = self.model(data)  # already physical
    ...
    def _check_convergence(self, epoch, Pi):
        # Pi is now the equilibrium residual (target = 0)
        loss = abs(Pi)  # always positive
        
        self._recent_Pi.append(loss)
        if len(self._recent_Pi) > self.cfg.smooth_window:
            self._recent_Pi.pop(0)

        if len(self._recent_Pi) >= self.cfg.smooth_window:
            smooth_loss = np.mean(self._recent_Pi)
            if smooth_loss < self.best_smooth_Pi - self.cfg.min_delta:
                self.best_smooth_Pi = smooth_loss
                self.best_epoch = epoch
                self.no_improve_count = 0
            else:
                self.no_improve_count += 1

        if loss < self.best_Pi:
            self.best_Pi = loss
            self._save_checkpoint('best.pt', epoch,
                                  {'loss': loss})
    ...
    def train(self):
        print(f"\n{'═'*95}")
        print(f"  TRAINING — Equilibrium Residual PIGNN")
        print(f"  {self.cfg.epochs} epochs, "
              f"lr={self.cfg.lr}, "
              f"grad_clip={self.cfg.grad_clip}, "
              f"device={self.device}")
        print(f"  Target: Loss → 0.0 (perfect equilibrium)")
        print(f"{'═'*95}")
        ...
        print(f"  Best Loss:     {self.best_Pi:.6e}")
    ...
    def _analyze_results(self):
        ...
        final_loss = abs(self.history['loss'][-1])
        print(f"\n  Final Loss:  {final_loss:.6e}")
        print(f"  Best  Loss:  {self.best_Pi:.6e}")
        print(f"  Target:      0.0")

        if self.best_Pi < 0.01:
            print(f"  ✓ CONVERGED (<1% residual)")
        elif self.best_Pi < 0.05:
            print(f"  ~ Close (<5%)")
        elif self.best_Pi < 0.1:
            print(f"  ~ Fair (<10%)")
        else:
            print(f"  ✗ Not converged")
        ...
        if valid_de:
            best_de = min(valid_de)
            print(f"\n  Best displacement error: {best_de:.4e}")
