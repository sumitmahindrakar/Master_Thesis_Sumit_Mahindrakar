...
class FrameEnergyLoss(nn.Module):
    ...
    def forward(self, model, data):
        ...
        # ═══════════════════════════════════════
        # Non-dimensionalize residual BEFORE squaring
        # This keeps gradients ~O(1)
        # ═══════════════════════════════════════
        if hasattr(data, 'batch') and data.batch is not None:
        ...
        res_nd = residual.clone()
        res_nd[:, 0] = residual[:, 0] / F_c
        res_nd[:, 1] = residual[:, 1] / F_c
        res_nd[:, 2] = residual[:, 2] / (F_c * data.L_c if not hasattr(data, 'batch') or data.batch is None 
                                          else data.F_c[data.batch] * data.L_c[data.batch])
        ...
        with torch.no_grad():
            ...
            ext_norm = (F_ext**2).sum().clamp(min=1e-30)
            accuracy = 1.0 - (residual**2).sum().sqrt() / ext_norm.sqrt()
