...
class PIGNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.final_norm = nn.LayerNorm(3 * H)

        # ── Separate decoder per DOF ──
        # self.decoder_ux = MLP([2 * H, H, 1])
        # self.decoder_uz = MLP([2 * H, H, 1])
        # self.decoder_th = MLP([2 * H, H, 1])
        self.decoder_uz = MLP([3 * H, H, 1])
        self.decoder_ux = MLP([3 * H, H, 1])
        self.decoder_th = MLP([3 * H, H, 1])
    ...
    def forward(self, data):
        ...
        # ── Global pooling ──
        g = global_mean_pool(h, data.batch)       # [n_graphs, H]
        g = self.global_proj(g)                    # [n_graphs, H]
        g_broadcast = g[data.batch]               # [N, H]

        z_local = self.final_norm(
            torch.cat([h, incoming, g_broadcast], dim=-1)  # [N, 3H]
        )
