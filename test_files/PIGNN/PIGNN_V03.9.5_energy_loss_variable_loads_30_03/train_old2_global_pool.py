...
class Config:
    ...
    # L-BFGS
    lbfgs_steps     = 2000     # long run
    lbfgs_lr        = 1.0
    lbfgs_history   = 500
    lbfgs_max_iter  = 20
    stall_patience  = 300       # restart after this many zero-steps
...
def train_lbfgs_with_restarts(
    ...
    loss_window = []
    err_window  = []
    WINDOW_SIZE = cfg.stall_patience           # check over last 50 steps
    ...
    for step in range(cfg.lbfgs_steps):
        ...
        if stalled and n_restarts < MAX_RESTARTS:
            ...
            # Increasing perturbation with each restart (capped)
            scale = min(
