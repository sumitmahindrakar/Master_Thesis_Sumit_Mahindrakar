...
class PIGNN(nn.Module):
    ...
    def __init__(self,
                 ...
                 edge_in_dim=10,
                 hidden_dim=128,
                 n_layers=6, # number of Hops # increased it to 15
                #  out_dim=3
                 ):
        super().__init__()
        H = hidden_dim
        ...
        self.mp_layers = nn.ModuleList([
            GraphNetworkLayer(H, H, aggr='add') # max pooling needed
    ...
    def forward(self, data):
        # ── Encode + MP + Decode (UNCHANGED) ──
        self._coords = data.coords.clone().requires_grad_(True)
        coords_norm = ((self._coords - data.coord_min) / data.coord_range)  # differentiable - for Autogard
        x = data.x.clone()
        # x[:, 0:3] = self._coords
        x[:, 0:3] = coords_norm  # balanced [0,1]
        ...
        raw = self.decoder(h)                    # (N, 15) all ~O(1)
        ...
        M_c     = data.M_c                                     # 
                                                            # 
        # Displacements: raw × u_c, θ × theta_c               # 
        pred[:, 0] = raw[:, 0] * u_c                           #   ux
        pred[:, 1] = raw[:, 1] * u_c                           #   uz
        pred[:, 2] = raw[:, 2] * theta_c                       #   θy
                                                            # 
        # Face forces: Fx,Fz × F_c, My × M_c                  # 
        for face in range(4):                                  # 
            base = 3 + face * 3                                # 
            pred[:, base + 0] = raw[:, base + 0] * F_c         #   Fx
            pred[:, base + 1] = raw[:, base + 1] * F_c         #   Fz
            pred[:, base + 2] = raw[:, base + 2] * M_c         #   My

        # ── Hard BC + Face mask (UNCHANGED) ──
        pred = self._apply_hard_bc(pred, data)
        pred = self._apply_face_mask(pred, data)
