...
class PIGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        M_c     = data.M_c                                     # ◀◀◀ NEW
                                                            # ◀◀◀ NEW
        # Displacements: raw × u_c, θ × theta_c               # ◀◀◀ NEW
        pred[:, 0] = raw[:, 0] * u_c                           # ◀◀◀ NEW  ux
        pred[:, 1] = raw[:, 1] * u_c                           # ◀◀◀ NEW  uz
        pred[:, 2] = raw[:, 2] * theta_c                       # ◀◀◀ NEW  θy
                                                            # ◀◀◀ NEW
        # Face forces: Fx,Fz × F_c, My × M_c                  # ◀◀◀ NEW
        for face in range(4):                                  # ◀◀◀ NEW
            base = 3 + face * 3                                # ◀◀◀ NEW
            pred[:, base + 0] = raw[:, base + 0] * F_c         # ◀◀◀ NEW  Fx
            pred[:, base + 1] = raw[:, base + 1] * F_c         # ◀◀◀ NEW  Fz
            pred[:, base + 2] = raw[:, base + 2] * M_c         # ◀◀◀ NEW  My
