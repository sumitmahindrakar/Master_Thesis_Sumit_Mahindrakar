...
Node inputs (9 per node):
  ...
  point_load       (3)  point load at node          ← CHANGED
  response_node    (1)  1=response measured here, 0=not

Element inputs (10 per element):
  length           (1)
  direction        (3)
  E, A, I22        (3)
  elem_load        (3)  qx, qy, qz
...
class FrameGraphBuilder:
    ...
    def build_node_features(self, case: dict) -> np.ndarray:
        ...
        feat[:, 5:8] = case['point_load'][:, :3]              # ← CHANGED
    ...
    def build_equivalent_nodal_loads(self,
        ...
        """
        Point loads → F_ext directly.
        point_load is (N, 3) in global [Fx, Fy, Fz].
        F_ext is (N, 3) as [Fx_global, Fz_global, My].
        """
        N = n_nodes
        F_ext = np.zeros((N, 3), dtype=np.float64)

        F_ext[:, 0] = point_load[:, 0]   # Fx global
        F_ext[:, 1] = point_load[:, 2]   # Fz global
        F_ext[:, 2] = 0.0                # No moment from point loads
...
def verify_graph(data: Data) -> bool:
    ...
    print(f"\n  5. NODE LOADS (point_load in x[:, 5:8]):")     # ← CHANGED
    pl = data.x[:, 5:8]
    loaded_nodes = torch.where(pl.abs().sum(dim=1) > 1e-10)[0]
    print(f"    Nodes with point load: {len(loaded_nodes)}/{N}")
    print(f"    Fz range: [{pl[:, 2].min():.4f}, "
          f"{pl[:, 2].max():.4f}]")
    ...
    if hasattr(data, 'F_c'):
        ...
        print(f"\n  8. PHYSICS SCALES: not yet computed")
...
def visualize_frame_graph(data: Data,
    ...
    for i in range(data.num_nodes):
        ...
        # Show point load arrow                               # ← CHANGED
        pl_z = x_feat[i, 7]
        if abs(pl_z) > 1e-10:
            ax.annotate('', xy=(xp, zp),
                       xytext=(xp, zp - pl_z * 0.02),
...
if __name__ == "__main__":
    ...
    if hasattr(batch, 'F_c'):
        ...
        print(f"  ✓ Physics scales accessible in batch")
    else:
    ...
    print(f"  Input x range: [{d_norm.x.min():.4f}, "
          f"{d_norm.x.max():.4f}]  (should be ~[0, 1])")
    print(f"  F_c still present: {hasattr(d_norm, 'F_c')}")
    print(f"  F_ext unchanged: [{d_norm.F_ext.min():.2f}, "
          f"{d_norm.F_ext.max():.2f}]  (raw units)")
    ...
    print(f"  │   [x,y,z, bc_d, bc_r, Px,Py,Pz, resp]   │")
