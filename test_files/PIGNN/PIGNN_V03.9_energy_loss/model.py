...
if __name__ == "__main__":
    ...
    print(f"    pred range: [{pred.min().item():.6e}, "
          f"{pred.max().item():.6e}]")
    print(f"    pred norm:  {pred.norm().item():.6e}")

    zero_ok = pred.abs().max().item() < 1e-6
    print(f"    {'✓' if zero_ok else '✗'} "
          f"Zero initialization verified")
    ...
    print(f"    {'✓' if has_grad else '✗'} "
          f"Gradients flow through all layers")
