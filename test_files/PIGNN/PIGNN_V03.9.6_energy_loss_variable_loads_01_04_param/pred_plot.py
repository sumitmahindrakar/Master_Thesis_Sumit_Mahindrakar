...
class PlotConfig:
    checkpoint_path = "RESULTS_800sample_Duplicate400/checkpoint.pt"
    norm_data_path  = "DATA_800sample_Duplicate400/graph_dataset_norm.pt"
    raw_data_path   = "DATA_800sample_Duplicate400/graph_dataset.pt"
    save_dir        = "PLOTS_800sample_Duplicate400"
...
class ResultsLoader:
    ...
    def load_all(self):
        ...
        # ─── FIX: handle all checkpoint formats ───
        if isinstance(ckpt, dict):
            if 'best_model_state' in ckpt:
                # train.py v5 format
                self.model.load_state_dict(ckpt['best_model_state'])
                epoch = 'best'
                self.ckpt_meta = ckpt  # keep metadata accessible
            elif 'final_model_state' in ckpt:
                self.model.load_state_dict(ckpt['final_model_state'])
                epoch = 'final'
                self.ckpt_meta = ckpt
        ...
        # ─── FIX: use correct per-DOF scaling (ux_c, uz_c, theta_c) ───
        # Determine uz scaling mode from checkpoint config
        use_ux_for_uz = True  # default
        if 'config' in self.ckpt_meta:
            use_ux_for_uz = self.ckpt_meta['config'].get(
                'use_ux_for_uz', True)
        ...
        with torch.no_grad():
            for ndata in self.norm_data:
                ...
                pp = torch.zeros_like(pr)
                pp[:, 0] = pr[:, 0] * nd.ux_c
                uz_scale = nd.ux_c if use_ux_for_uz else nd.uz_c
                pp[:, 1] = pr[:, 1] * uz_scale
                pp[:, 2] = pr[:, 2] * nd.theta_c
                self.predictions.append(pp.cpu())
        print(f"  Predictions: {len(self.predictions)}")
        print(f"  uz scaling: {'ux_c' if use_ux_for_uz else 'uz_c'}")
        ...
        ranges = [x_range, y_range, z_range]
        sorted_axes = np.argsort(ranges)[::-1]
        self.h_axis = sorted_axes[0]
        self.v_axis = sorted_axes[1]
