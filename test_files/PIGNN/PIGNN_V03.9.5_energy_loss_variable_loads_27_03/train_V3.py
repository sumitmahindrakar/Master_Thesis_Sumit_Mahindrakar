...
class TrainConfig:
    ...
    resume_from     = "RESULTS/final.pt"  # or "RESULTS/final.pt" "RESULTS/best.pt" 
    ...
    warmup_epochs   = 0 # 0 -> No warm up needed for training the trained model
    decay_epoch     = 2000
    lr_decay_factor = 0.1
    weight_decay    = 0.0

    # ── Gradient clipping ──
    grad_clip       = 200.0 #500.0    # clip gradient norm
    batch_size      = 17 #4 #32     # 32 (total 400- training 350- 350/16=22 batches per epoch)

    # Axial curriculum
    axial_ramp_start = 1         # epoch to start ramping
    axial_ramp_end   = 1500      # epoch at which axial_weight = 1.0
...
class Trainer:
    ...
    def train_one_epoch(self, epoch):
        ...
        # ═══ Axial curriculum weight ═══
        cfg = self.cfg
        if epoch <= cfg.axial_ramp_start:
            axial_weight = 0.0
        elif epoch >= cfg.axial_ramp_end:
            axial_weight = 1.0
        else:
            axial_weight = (epoch - cfg.axial_ramp_start) / (cfg.axial_ramp_end - cfg.axial_ramp_start)
        ...
        for batch in self.train_loader:
            ...
            Pi_norm, loss_dict, pred_raw, u_phys = \
                self.loss_fn(self.model, batch, axial_weight=axial_weight)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in self.test_data:
                ...
                pred_phys = torch.zeros_like(pred_raw)
                pred_phys[:, 0] = pred_raw[:, 0] * data.ux_c     # was u_c
                pred_phys[:, 1] = pred_raw[:, 1] * data.uz_c     # was u_c
