...
class Config:
    ...
    # Multiple seeds
    n_seeds         = 3        # try 3 random initializations
...
def train_lbfgs_with_restarts(
    ...
    """
    L-BFGS training with automatic restarts when stalled.
    
    Detects stalls by monitoring loss change. When loss stops
    changing for `stall_patience` consecutive steps, resets
    L-BFGS optimizer (clears history), forcing it to restart
    ...
    for step in range(cfg.lbfgs_steps):
        ...
        try:
            ...
            print(f"    Step {step}: L-BFGS exception — {e}")
            optimizer = make_optimizer()
            n_restarts += 1
            stall_count = 0
            continue
        ...
        if not np.isfinite(loss_val[0]):
            print(f"    Step {step}: NaN — restarting")
        ...
        # ── Stall detection ──
        loss_change = abs(loss_val[0] - prev_loss)
        if loss_change < cfg.stall_tol:
            stall_count += 1
        else:
            stall_count = 0
        prev_loss = loss_val[0]

        # ── Auto-restart when stalled ──
