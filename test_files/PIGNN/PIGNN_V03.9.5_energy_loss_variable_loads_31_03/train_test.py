"""
=================================================================
train.py — Full Pipeline PIGNN Training v4
=================================================================
Improvements over v3:
  1. Unified TrainingHistory across ALL phases (Adam→LBFGS→Polish)
  ...
  7. Centralized uz_c scaling flag (no more scattered hacks)
...
class TrainingHistory:
    """
    Single history object spanning all training phases.
    Each record is a dict with a monotonic global_step,
    phase label, and whatever metrics were logged.
    """
...
def _log_line(phase_tag, local_step, ld, err_tr, err_te,
              gn, lr, pred, extra=""):
    print(
        f"    {local_step:5d} | "
        f"Π={ld['Pi']:+.4e} | "
        ...
        f"|∇|={gn:.2e} | "
        f"lr={lr:.2e} | "
        f"pred=[{pred.min().item():.3f},{pred.max().item():.3f}]"
        f"{extra}"
    )
...
def train_adam_minibatch(model, train_graphs, train_batch, test_batch,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=max(cfg.adam_phase1_epochs, 1),   # ← never zero
    ...
    for epoch in range(cfg.adam_phase1_epochs):
        ...
        for mb in loader:
            ...
            optimizer.zero_grad()
            pred = model(mb)
            loss, _, _, _ = loss_fn.compute(pred, mb, axial_weight=aw)
            loss.backward()
        ...
        _log_line('A1', epoch, ld_tr, err_tr, err_te,
                  gn, lr, pred_tr)
...
# ════════════════════════════════════════════════
# Phase  — Focused on uz only adn freeze other ux and theta
# ════════════════════════════════════════════════

def train_uz_focused(model, train_graphs, loss_fn, optimal_aw, cfg, device):
    """Phase 2: Freeze all except decoder_uz, train with energy loss."""
    model.to(device).train()

    # Freeze everything except decoder_uz
    for name, param in model.named_parameters():
        if 'decoder_uz' in name:
    ...
    print(f"    Trainable: {n_trainable}/{n_total} params (decoder_uz only)")
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=cfg.uz_epochs, eta_min=cfg.uz_lr / 100
    )
    ...
    print(f"\n    uz-focused done: best_err={best_err:.4f}")
    return best_err, best_state

    
# ════════════════════════════════════════════════
# Phase 2 — L-BFGS with auto-restart
# ════════════════════════════════════════════════
...
def train_lbfgs_with_restarts(model, train_batch, test_batch,
    ...
    # ── Guard: skip if zero steps ──
    if cfg.lbfgs_steps <= 0:
        print("    Phase 2: SKIPPED (0 steps)")
        with torch.no_grad():
            p = model(train_batch)
    ...
    for step in range(cfg.lbfgs_steps):
        ...
        def closure():
            ...
            loss, _, _, _ = loss_fn.compute(pred, train_batch,
                                            axial_weight=aw)
            loss.backward()
            lv[0] = loss.item()
            return loss
        ...
        if step % cfg.log_every == 0 or step < 5:
            ...
            _log_line('LB', step, ld_tr2, err_tr2, err_te,
    ...
    if best_state is not None:
        model.load_state_dict(best_state)
    print(f"\n    Phase 2 done: {elapsed:.0f}s, {n_restarts} restarts, "
...
def adam_polish(model, train_batch, test_batch,
    ...
    if cfg.adam_epochs <= 0:
        print("    Phase 3: SKIPPED (0 epochs)")
        with torch.no_grad():
            p = model(train_batch)
        err = compute_disp_error(p, train_batch, cfg)
        return err[0], 0.
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=max(cfg.adam_epochs, 1),   # ← never zero
    ...
    for epoch in range(cfg.adam_epochs):
        optimizer.zero_grad()
        pred = model(train_batch)
        loss, _, _, _ = loss_fn.compute(pred, train_batch, axial_weight=aw)
        loss.backward()
        gn = _grad_norm(model)
        clip_grad_norm_(model.parameters(), cfg.grad_clip)
        optimizer.step()
        ...
        _record_metrics(history, epoch, loss_tr, loss_te,
                        err_tr, err_te, ld_tr, ld_te,
                        gn, lr, pred_tr, best_err)
        _log_line('PL', epoch, ld_tr, err_tr, err_te,
                  gn, lr, pred_tr)
    ...
    if best_state is not None:
        model.load_state_dict(best_state)
    print(f"\n    Phase 3 done: {elapsed:.0f}s, best_err={best_err:.4f}")
...
def plot_curves(history, save_dir, best_err, target_Pi):
    ...
    plt.suptitle(f"Training Curves — best err={best_err:.4f}",
...
def main():
    ...
    print("=" * 70)
    print("  PIGNN TRAINING — Full Pipeline v4")
    ...
    for seed in range(cfg.n_seeds):
        ...
        # ── Phase 2 ──
        print(f"\n    ── Phase 2: L-BFGS ──")
        e2, l2, t2 = train_lbfgs_with_restarts(
    ...
    # ── Handle case where no training happened at all ──
    if best_overall_state is None:
        ...
        best_times = {'adam_phase1': 0., 'lbfgs': 0., 'adam_polish': 0.}
    ...
    print(f"    ► scales, data_split, timings")
