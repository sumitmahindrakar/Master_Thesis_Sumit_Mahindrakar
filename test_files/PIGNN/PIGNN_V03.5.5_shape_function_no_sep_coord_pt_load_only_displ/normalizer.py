...
class PhysicsScaler:
    ...
    def compute_and_store(data: Data) -> Data:
        ...
        coords = data.coords
        x_span = (coords[:, 0].max() - coords[:, 0].min()).item()
        z_span = (coords[:, 2].max() - coords[:, 2].min()).item()
        L_c = max(x_span, 1e-10)     
        ...
        u_c     = F_c * L_c ** 3 / (100.0 * EI_c)
        theta_c = F_c * L_c ** 2 / (100.0 * EI_c)
        ...
        data.L_elem_max = torch.tensor(
            data.elem_lengths.max().item(), dtype=torch.float32
        )
