...
class Trainer:
    ...
    def _load_data(self):
        ...
        # ══════════════════════════════════════════════
        # VERIFICATION: coord_min and coord_range
        # ══════════════════════════════════════════════
        d = self.data_list[0]

        if hasattr(d, 'coord_min') and hasattr(d, 'coord_range'):
            print(f"\n  Coordinate normalization params:")
            print(f"    coord_min:   {d.coord_min}")
            print(f"    coord_range: {d.coord_range}")
            print(f"    coords raw:  [{d.coords.min():.2f}, {d.coords.max():.2f}]")
            
            # Test normalized coords
            cr = d.coord_range.clamp(min=1e-8)
            coords_norm = (d.coords - d.coord_min) / cr
            print(f"    coords norm: [{coords_norm.min():.4f}, {coords_norm.max():.4f}]"
                f"  (should be ~[0, 1])")
            print(f"    ✓ coord_min and coord_range present")
        else:
            print(f"\n  ✗ coord_min / coord_range NOT FOUND!")
            print(f"    Re-run Step 2 graph construction with normalizer")
            raise RuntimeError("Missing coord_min / coord_range in data")               
        ...
        print(f"    θ_c  = {d.theta_c.item():.4e}  (char. rot)")
