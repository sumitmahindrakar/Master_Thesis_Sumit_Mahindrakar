...
EVERYTHING ELSE UNCHANGED:
  Same 15-value output, same face forces, same autograd.
=================================================================
"""
...
class NaivePhysicsLoss(nn.Module):
    ...
    # ════════════════════════════════════════════════════════
    # SECTION B: EQUILIBRIUM LOSSES — NOW NON-DIMENSIONALIZED
    # ════════════════════════════════════════════════════════
    ...
    def _loss_equilibrium(self, face_forces, F_ext, bc_disp, bc_rot,
        ...
        # Non-dim: forces by F_c, moments by M_c       # ◀◀◀ NEW
        res_Fx = residual[free_mask, 0] / F_c           # ◀◀◀ NEW
        res_Fz = residual[free_mask, 1] / F_c           # ◀◀◀ NEW
        res_My = residual[free_mask, 2] / M_c           # ◀◀◀ NEW
    ...
    # ════════════════════════════════════════════════════════
    # SECTION C: CONSTITUTIVE LOSSES — NOW NON-DIMENSIONALIZED
    # ════════════════════════════════════════════════════════

    # def _compute_autograd_derivs(self, pred, coords):
    def _compute_autograd_derivs(self, pred, coords, data):
        """C0. Unchanged — same autograd derivatives."""
        ux = pred[:, 0]
        uz = pred[:, 1]

        # Chain rule scale factors
        cr = data.coord_range.to(coords.device).clamp(min=1e-8)
        sx = 1.0 / cr[0]  # ~1/18
        sz = 1.0 / cr[2]  # ~1/18
        ...
        # 2nd derivatives
        grad_duz_dx = torch.autograd.grad(
            grad_uz[:, 0].sum(), coords, create_graph=True, retain_graph=True)[0]
        d2uz_dx2 = grad_duz_dx[:, 0] * sx * sx

        grad_dux_dz = torch.autograd.grad(
            grad_ux[:, 2].sum(), coords, create_graph=True, retain_graph=True)[0]
        d2ux_dz2 = grad_dux_dz[:, 2] * sz * sz

        # 3rd derivatives
        grad_d2uz = torch.autograd.grad(
            grad_duz_dx[:, 0].sum(), coords, create_graph=True, retain_graph=True)[0]
        d3uz_dx3 = grad_d2uz[:, 0] * sx * sx * sx

        grad_d2ux = torch.autograd.grad(
            grad_dux_dz[:, 2].sum(), coords, create_graph=True, retain_graph=True)[0]
        d3ux_dz3 = grad_d2ux[:, 2] * sz * sz * sz
    ...
    def forward(self, model, data):
        """
        Same flow as before.
        Only change: all sub-losses now receive `data` for scales.
        """
        # 1. Forward
        pred = model(data)
        ...
        # 3. Equilibrium (pass data for scales)           # ◀◀◀ CHANGED
        ...
        # 5. Constitutive (pass data for scales)          # ◀◀◀ CHANGED
        # derivs = self._compute_autograd_derivs(pred, coords)
        derivs = self._compute_autograd_derivs(pred, coords, data)
