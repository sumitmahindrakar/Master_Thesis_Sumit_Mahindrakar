...
#     This creates a PINN-like continuous field per element,
#     conditioned on the GNN's understanding of the global structure.
...
#         super().__init__()
#         # Input: [ξ, h_i, h_j] → 1 + 2*hidden_dim
#         self.net = nn.Sequential(
#             nn.Linear(1 + 2 * hidden_dim, 64),
#             nn.SiLU(),
#             nn.Linear(64, 64),
#             nn.SiLU(),
#             nn.Linear(64, n_dofs),  # [u_axial, w_transverse, θ]
#         )
...
#             h_i: (n_points, H) source node embedding (broadcast OK)
#             h_j: (n_points, H) target node embedding (broadcast OK)
...
#         3. Autograd → du/dξ, d²w/dξ², d³w/dξ³
#         4. Physics loss computed from derivatives
    
#     Hard BCs: enforce u(ξ=0)=0 or u(ξ=1)=0 at support nodes
#     """
...
#         # ── GNN: processes graph → node embeddings ──
...
#         # ── Edge Field MLP: ξ + embeddings → displacement field ──
...
#         h_i_exp = h_i.unsqueeze(1).expand(-1, n_pts, -1)  # (E, n_pts, H)
#         h_j_exp = h_j.unsqueeze(1).expand(-1, n_pts, -1)  # (E, n_pts, H)

#         # Flatten for MLP
#         xi_flat = xi.reshape(-1, 1)            # (E*n_pts, 1)
#         h_i_flat = h_i_exp.reshape(-1, self.hidden_dim)  # (E*n_pts, H)
#         h_j_flat = h_j_exp.reshape(-1, self.hidden_dim)  # (E*n_pts, H)

#         # Evaluate field
#         fields_flat = self.edge_field(xi_flat, h_i_flat, h_j_flat)
#         fields = fields_flat.reshape(E_num, n_pts, 3)  # (E, n_pts, 3)

#         # ── Hard BCs ──
#         # If node i is a support → field at ξ=0 must be zero
#         # If node j is a support → field at ξ=1 must be zero
#         # Multiply by soft ramp near boundaries for supported nodes
#         if bc_mask is not None:
#             bc_i = bc_mask[n1]  # (E,) 1 if node i is support
#             bc_j = bc_mask[n2]  # (E,) 1 if node j is support

#             # Ramp functions that go to 0 at supported ends
#             # ξ goes from 0 (node i) to 1 (node j)
#             xi_vals = xi.squeeze(-1)  # (E, n_pts)

#             # If node i is fixed: multiply by ξ (goes to 0 at ξ=0)
#             # If node j is fixed: multiply by (1-ξ) (goes to 0 at ξ=1)
#             mask_i = 1.0 - bc_i.unsqueeze(1) * (1.0 - xi_vals)
#             # when bc_i=1: mask = ξ (zero at ξ=0)
#             # when bc_i=0: mask = 1 (no effect)

#             mask_j = 1.0 - bc_j.unsqueeze(1) * xi_vals
#             # when bc_j=1: mask = (1-ξ) (zero at ξ=1)
#             # when bc_j=0: mask = 1 (no effect)
...
#             I22_grad:  (n_elem,)       I22 with requires_grad for sensitivity
#         """
#         # 1. GNN encoding
#         h = self.encode(data)
...
#         xi_1d = torch.linspace(0, 1, n_quad_pts,
#                             device=data.x.device)
#         xi = xi_1d.unsqueeze(0).unsqueeze(-1).expand(
#             E_num, -1, -1).clone()
#         xi.requires_grad_(True)

#         # 3. Evaluate displacement field
#         bc_mask = data.bc_disp.squeeze(-1)
#         fields = self.evaluate_field(
#             h, data.connectivity, xi, bc_mask)

#         # 4. Create gradient-tracked I22
