...
class PhysicsScaler:
    ...
    def compute_and_store(data: Data, 
        ...
        # ═══ FIX 1: Create bc_mask from bc_disp ═══
        if hasattr(data, 'bc_disp') and not hasattr(data, 'bc_mask'):
            data.bc_mask = data.bc_disp.squeeze(-1).to(dtype=torch.bool, device=device)
        ...
        # data.theta_c = torch.tensor(theta_c, dtype=torch.float32, device=device)
            if data.y_node is not None:
                ux_c = max(data.y_node[:, 0].abs().max().item(), 1e-15)
                uz_c = max(data.y_node[:, 1].abs().max().item(), 1e-15)
                u_c  = max(ux_c, uz_c)
                theta_c = max(data.y_node[:, 2].abs().max().item(), 1e-15)
            else:
                ux_c = F_c * L_c**3 / EI_c
        ...
        for name, val in [('F_c',F_c), ('M_c',M_c), ('ux_c',ux_c),
                        ('uz_c',uz_c), ('u_c',u_c), ('theta_c',theta_c),
                        ('L_c',L_c), ('EI_c',EI_c), ('EA_c',EA_c)]:
            setattr(data, name,
                    torch.tensor(max(val,1e-15), dtype=torch.float32, device=device))
        data.q_c = torch.tensor(F_c/L_c, dtype=torch.float32, device=device)
    ...
    def compute_global_scales(data_list: list) -> dict:
        ...
        # ═══ FIX 2: Separate scales for each DOF ═══
        ux_c = max(
            d.y_node[:, 0].abs().max().item()
            for d in data_list if d.y_node is not None
        )
        uz_c = max(
            d.y_node[:, 1].abs().max().item()
            for d in data_list if d.y_node is not None
        )
        theta_c = max(
        ...
        scales = {
            ...
            'ux_c':    max(ux_c, 1e-15),
            'uz_c':    max(uz_c, 1e-15),
            'u_c':     max(ux_c, uz_c, 1e-15),   # backward compat
            'theta_c': max(theta_c, 1e-15),
        }
        ...
        for k, v in scales.items():
            print(f"    {k:<10} = {v:.4e}")
        print(f"    ux_c/uz_c ratio = {ux_c/max(uz_c,1e-30):.1f}×")
