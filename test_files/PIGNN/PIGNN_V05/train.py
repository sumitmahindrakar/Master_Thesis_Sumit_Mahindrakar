...
def verify_autodiff(model, sample, device):
    ...
    # ── Jacobian diagonal check (small graph only) ──
    N = pred.shape[0]
    if N <= 50:
        print(f"\n  6. JACOBIAN DIAGONAL CHECK (N={N}):")
        u_z_col = pred[:, 1]
        n_check = min(5, N)
        off_diag_ratios = []
        ...
        for i in range(n_check):
            ...
            other_grad = torch.cat([g[:i, 0], g[i+1:, 0]]).abs().max().item()

            if self_grad > 1e-15:
                ratio = other_grad / self_grad
                off_diag_ratios.append(ratio)
                print(f"     Node {i}: self={self_grad:.4e}, "
                      f"max_other={other_grad:.4e}, "
                      f"ratio={ratio:.4e}"
                      f" {'✓' if ratio < 0.01 else '⚠'}")
            else:
                print(f"     Node {i}: self=~0, skip")

        if off_diag_ratios:
            avg_ratio = sum(off_diag_ratios) / len(off_diag_ratios)
            print(f"     Average off-diag/diag ratio: {avg_ratio:.4e}")
            print(f"     {'✓ Block-diagonal!' if avg_ratio < 0.01 else '⚠ Cross-terms detected'}")
    else:
        print(f"\n  6. JACOBIAN CHECK: skipped (N={N} too large)")
