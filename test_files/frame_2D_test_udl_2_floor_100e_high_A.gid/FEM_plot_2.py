...
VTK_FILE_PATH = "test_files/frame_2D_test_udl_2_floor_100e_high_A.gid/vtk_output/Parts_Beam_Beams_0_1.vtk"  # <-- Change this to your VTK file path
...
# --- Line Widths ---
LINEWIDTH_STRUCTURE = 1.0
LINEWIDTH_DEFORMED = 1.0
LINEWIDTH_DIAGRAM = 1.0
LINEWIDTH_THIN = 0.4

# --- Figure Settings ---
FIGURE_DPI = 100
SAVE_FIGURES = True
OUTPUT_FOLDER = "test_files/frame_2D_test_udl_2_floor_100e_high_A.gid/plots"
# --- Support Settings ---
...
def create_individual_plots(vtk_data, output_folder=OUTPUT_FOLDER):
    ...
    fig, ax = plt.subplots(figsize=(10, 8), dpi=FIGURE_DPI)
    setup_axes(ax, f'Deformed Shape (Scale: {DEFORMED_SCALE}x)', points)
    plot_structure(ax, points, cells, color='lightgray', linewidth=1.0, label='Original')
    plot_deformed_shape(ax, points, cells, displacement, scale=DEFORMED_SCALE)
    add_supports(ax, points)
    ax.legend(loc='best')
